...
> **도구 표기 주의**: 본 스킬은 unity-mcp 채널만 사용한다. 이 문서는 Editor 호출이 거의 없고 Figma MCP (`use_figma` / `get_metadata`) 위주이므로 unity-cli 잔존 표기는 없다. Editor 측 호출이 필요한 경우 SKILL.md Phase 0 매핑 표를 따른다.

FigmaToUnity 가 Figma 문서를 Unity UGUI 로 일괄 임포트하기 전에, **에이전트가 Figma MCP 로 1차 정리**를 수행해야 한다.
