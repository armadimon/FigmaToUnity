# (참고용) 구버전 unity-cli Commands Reference

> **본 스킬은 unity-cli 를 호출하지 않는다.** 이 문서는 **참고용 자료** 로 유지되며, 패키지 사용자가 구 unity-cli 환경에서 폴백을 원할 때만 의미가 있다. 실제 실행은 SKILL.md Phase 0 매핑 표에 따라 모두 `mcp__unity-mcp__Unity_RunCommand` (C# 스니펫) 로 변환한다.
>
> 핵심 매핑:
>
> - `ui canvas.create` → `new GameObject(name, typeof(Canvas), typeof(CanvasScaler), typeof(GraphicRaycaster))` + 속성 대입
> - `ui panel.create / ui image.create` → `new GameObject(name, typeof(RectTransform), typeof(CanvasRenderer), typeof(Image))` + RectTransform/Image 속성 대입
> - `ui text.create` → `new GameObject(name, typeof(RectTransform), typeof(CanvasRenderer), typeof(TextMeshProUGUI))` + 속성 대입
> - `ui button.create` → 위 + `typeof(Button)` + 자식 텍스트
> - `ui layout.add` → `AddComponent<HorizontalLayoutGroup/VerticalLayoutGroup/GridLayoutGroup/ContentSizeFitter>()` + 속성 대입
> - `ui recttransform.modify` → RectTransform 의 anchor/pivot/anchoredPosition/sizeDelta 직접 대입
> - `editor gameview.resize` → unity-mcp 환경 공개 API 없음 → `Capture Default Screen` 메뉴 / 임시 카메라+RT 로 우회
> - `ui screenshot.capture width=W height=H` → `Capture Default Screen` 메뉴 또는 `Unity_RunCommand` 안의 RT 캡처 + `File.WriteAllBytes`
> - `asset import-texture path=... textureType=Sprite` → `AssetDatabase.ImportAsset(path)` + `TextureImporter.textureType = TextureImporterType.Sprite`

UGUI from Screenshot 스킬에 필요한 추가 unity-cli 명령어 (구 환경 참고).
