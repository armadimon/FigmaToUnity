...
> **도구 표기 주의**: 본 스킬은 unity-mcp 채널만 사용한다. 본 문서에 남은 `unity-cli ...` 표기는 모두 SKILL.md Phase 0 의 매핑 표대로 `mcp__unity-mcp__Unity_RunCommand` 안의 C# 스니펫 (`EditorApplication.ExecuteMenuItem(...)` 등) 으로 변환해서 실행한다.

FigmaToUnity 가 가져온 텍스트는 **TextMeshPro Essential Resources** 에 포함된 `LiberationSans SDF` 와 그 fallback 만으로 렌더된다.
...
### 2단계: 부트스트랩 메뉴 실행 (unity-mcp `Unity_RunCommand`)
