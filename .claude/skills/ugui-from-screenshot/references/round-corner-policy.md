...
> **도구 표기 주의**: 본 스킬은 unity-mcp 채널만 사용한다. 이 문서는 정책 선언 위주이며 unity-cli 잔존 표기가 없다. 실제 Editor 호출이 필요한 경우 SKILL.md Phase 0 매핑 표에 따라 `mcp__unity-mcp__Unity_RunCommand` 로 변환해 실행한다.
