...
> **도구 표기 주의**: 본 스킬은 unity-mcp 채널만 사용한다. 본 문서에 등장하는 `unity-cli editor gameview.resize` / `unity-cli ui screenshot.capture` 는 모두 unity-mcp 환경에서 다음으로 변환한다:
>
> - 강제 W×H 캡처: **부트스트랩의 `Capture Default Screen` 메뉴를 `Unity_RunCommand` 의 `EditorApplication.ExecuteMenuItem(...)` 로 호출** (prefab.sizeDelta 그대로 캡처). 임의 W×H 가 필요하면 `Unity_RunCommand` 안에서 임시 카메라+RT 로 직접 렌더 후 `File.WriteAllBytes(path, bytes)` 저장. 또는 사전 등록된 `Tools/UguiFromScreenshot/Capture Game View {W}x{H}` 메뉴 호출.
> - GameView 종횡비 변경: unity-mcp 환경에서는 공개 API 가 없으므로 사용자에게 GameView 종횡비 변경 요청 또는 위 RT 기반 캡처로 우회.
>
> 본문에 남은 `unity-cli ... outputPath=...` 명령은 모두 위 패턴을 호출 단위로 옮겨서 사용한다.
