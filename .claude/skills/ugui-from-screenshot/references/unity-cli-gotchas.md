# Unity-CLI Gotchas (구 환경 시행착오) + unity-mcp 등가 변환 노트

> **본 스킬은 unity-cli 를 호출하지 않는다.** 이 문서는 구 unity-cli 시대의 시행착오를 보존하되, 동일 함정이 unity-mcp 환경에서도 발생하는지/어떻게 변환되는지 안내한다. SKILL.md Phase 0 매핑 표를 함께 본다.
>
> 동일 함정 (unity-mcp 환경에서도 살아있음):
>
> - **GameObject 이름 중복 시 `Find` 가 첫 번째만 반환** → `GameObject.Find` 결과를 신뢰하지 말고 부모 transform 을 거쳐 자식 탐색.
> - **에셋 → 씬 인스턴스화 시 부모 누락** → `PrefabUtility.InstantiatePrefab(prefab, parent)` 의 두 번째 인자에 부모 transform 을 반드시 전달.
> - **쉘 이스케이프** 는 unity-mcp 에서는 발생하지 않음 (C# 안에서 문자열 직접 대입).
> - **`Assets/Refresh` / `editor compile` 호출 시 브릿지 일시 끊김** → unity-mcp 에서도 `AssetDatabase.Refresh()` 호출 직후 한동안 응답이 늦을 수 있음. 컴파일 후 `Unity_GetConsoleLogs` 로 폴링.
>
> 사라진 함정 (unity-mcp 에서는 발생하지 않음):
>
> - `ui.*.create` 의 `parentName` 무시 → 해당 없음 (C# 에서 부모 transform 직접 받음)
> - `gameobject reparent success=true 인데 실제 적용 X` → 해당 없음 (Undo.SetTransformParent 직접 호출)
> - `unity-cli tool list` 로 명령 지원 여부 확인 → 해당 없음 (Unity API 가 직접 노출됨)

실제로 겪은 시행착오 기반으로 unity-cli 사용 시 반드시 숙지할 내용 (구 unity-cli 환경 기준).
