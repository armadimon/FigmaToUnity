# FigmaToUnity 일괄 임포트 워크플로우

> **도구 표기 주의**: 본 스킬은 unity-mcp 채널만 사용한다. 본 문서에 등장하는 `unity-cli status` / `unity-cli menu execute path=...` / `unity-cli console clear` / `unity-cli --json console get` / `run_unity_to_figma_sync.sh` 호출은 모두 SKILL.md Phase 0 매핑 표에 따라 다음으로 변환한다:
>
> - `unity-cli status` → `Unity_GetConsoleLogs(maxEntries=1)` 응답 여부 확인
> - `unity-cli menu execute path="X"` → `Unity_RunCommand` 안에서 `EditorApplication.ExecuteMenuItem("X")`
> - `unity-cli console clear` → `Unity_RunCommand` 안에서 `UnityEditorInternal.LogEntries.Clear()` (reflection)
> - `unity-cli --json console get` → `Unity_GetConsoleLogs(logTypes="Log,Warning,Error", maxEntries=200, includeStackTrace=true)` + 메시지 패턴 매칭
> - `run_unity_to_figma_sync.sh` → SKILL.md Step 1A.3 의 `Unity_RunCommand` 스니펫 (ContextFile JSON 작성 + EditorPrefs/PlayerPrefs 저장 + Sync 메뉴 실행을 한 번에 처리)
>
> 또한 패키지 이름은 본 리포에서 `armadimon/FigmaToUnity` (구 `geuneda/UnityToFigma` / `com.simonoliver.unitytofigma`) 로 통일한다. Editor 측 메뉴 경로는 호환을 위해 `Tools/UnityToFigma Bootstrap/*` 그대로 유지.

이 문서는 Figma URL 입력 시 사용하는 **1차 자동 임포트 경로**를 정리한다.
에이전트가 노드를 하나씩 분석/생성하지 않고, [FigmaToUnity](https://github.com/armadimon/FigmaToUnity) 패키지로 한 번에 가져온 뒤
...
| 폰트 매칭 실패 | Console 로그에 `[UnityToFigma]` 로 남음 + `FigmaMissingFontTracker` 가 누적 | Sync 직후 자동으로 `FigmaMissingFontWindow` 가 떠서 사용자가 ObjectField 로 직접 폰트 선택. 한글 누락이면 추가로 `Setup TMP Korean Fallback` 호출. AI 가 `font` 프로퍼티를 임의 교체하지 않는다. |
...
추가 메시지는 `[UnityToFigma]` 접두로 들어온다 (다운로드 실패 등). 이 줄은 그대로 사용자 보고서에 포함한다. 폰트 매칭 실패 family 는 `FigmaMissingFontTracker` 가 자동 누적해서 `FigmaMissingFontWindow` 가 사용자에게 직접 보여준다 (위 표 참조).
...
2. 보정이 필요한 항목은 다음으로 한정:
   - 폰트 매칭 실패 → AI 가 자동 교체하지 않음. 패키지의 `FigmaMissingFontWindow` 가 사용자에게 직접 선택받는 GUI 를 띄움.
...
| 폰트가 깨져 보임 | 한글이면 `Setup TMP Korean Fallback` 호출. 그 외 family 는 `UnityToFigma/Postprocess/Open Missing Font Window` 에서 사용자가 ObjectField 로 직접 선택. AI 가 임의로 `font` 프로퍼티를 교체하지 않는다. |
