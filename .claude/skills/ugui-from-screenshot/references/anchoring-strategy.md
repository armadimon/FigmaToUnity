...
> **도구 표기 주의**: 본 스킬은 unity-mcp 채널만 사용한다. 본 문서 곳곳에 `CLI:` 라벨로 `unity-cli ui panel.create ...` 같은 명령이 적혀 있지만, 실제 실행은 **`mcp__unity-mcp__Unity_RunCommand`** 안의 C# 스니펫으로 처리한다. unity-cli → unity-mcp 의 동등 변환 패턴은 SKILL.md Phase 0 의 매핑 표 + 본 문서 Section "## Top Bar 의 unity-mcp 호출 예시" 참조.
>
> 일반 규칙: `ui panel.create` ≈ `new GameObject(..., typeof(Image))` + RectTransform 설정 / `ui layout.add` ≈ `AddComponent<*LayoutGroup>` + 속성 대입 / `ui recttransform.modify` ≈ RectTransform 직접 대입.
...
## Top Bar 의 unity-mcp 호출 예시 (이 패턴을 다른 모든 항목에 적용한다)
...
internal class CommandScript : IRunCommand
{
    ...
    {
        var canvas = GameObject.Find("UICanvas")?.GetComponent<RectTransform>();
        if (canvas == null) { result.LogError("UICanvas 없음"); return; }

        var go = new GameObject("Header", typeof(RectTransform), typeof(CanvasRenderer), typeof(Image));
        ...
        result.Log("Header anchor={0}~{1} size={2}", rt.anchorMin, rt.anchorMax, rt.sizeDelta);
    }
}
```

이후 문서에 나오는 `unity-cli ui panel.create ...` 예시는 위 패턴에서 이름·앵커·사이즈·색상만 바꿔서 사용한다. `unity-cli ui layout.add layoutType=Horizontal/Vertical/Grid/ContentSizeFitter` 는 각각 `AddComponent<HorizontalLayoutGroup>()` / `VerticalLayoutGroup` / `GridLayoutGroup` / `ContentSizeFitter` + 속성 대입.
