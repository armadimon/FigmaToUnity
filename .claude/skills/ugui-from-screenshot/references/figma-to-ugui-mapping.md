...
> **도구 표기 주의**: 본 스킬은 unity-mcp 채널만 사용한다. 이 문서에 남은 `unity-cli ...` 표기는 모두 SKILL.md Phase 0 매핑 표에 따라 `mcp__unity-mcp__Unity_RunCommand` 안의 C# 스니펫으로 변환해서 실행한다. 예: `unity-cli editor refresh` → `AssetDatabase.Refresh()` (프리팹 편집 중 금지), `unity-cli asset import-texture path=... textureType=Sprite` → `AssetDatabase.ImportAsset(path, ImportAssetOptions.ForceUpdate)` + `TextureImporter.textureType = TextureImporterType.Sprite`, `unity-cli ui image.create ... spritePath=...` → `new GameObject(..., typeof(Image)).GetComponent<Image>().sprite = AssetDatabase.LoadAssetAtPath<Sprite>(spritePath)`.
