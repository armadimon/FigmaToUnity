...
namespace UnityToFigma.Editor.FigmaApi
{
    ...
    public static class FigmaDataUtils
    {
        ...
        /// <summary>
        /// Returns the CANVAS (page) node id under the document that contains <paramref name="node"/>, or empty if not found.
        /// </summary>
        public static string GetPageCanvasIdContainingNode(FigmaFile file, Node node)
        {
            if (file?.document?.children == null || node == null)
                return string.Empty;

            foreach (var page in file.document.children)
            {
                if (NodeIsInSubtree(page, node))
                    return page.id;
            }
        ...
        /// <summary>
        /// Returns the ids of nodes that will currently be treated as screens during import.
        /// </summary>
        public static HashSet<string> GetScreenNodeIds(FigmaFile file, IEnumerable<Node> selectedPages = null)
        {
            var screenNodeIds = new HashSet<string>();
            if (file?.document?.children == null)
                return screenNodeIds;

            var pages = selectedPages?.ToList() ?? file.document.children.ToList();
            foreach (var page in pages)
            {
                CollectScreenNodeIds(page, page, screenNodeIds);
            }

            return screenNodeIds;
        }

        static void CollectScreenNodeIds(Node node, Node parentNode, HashSet<string> screenNodeIds)
        {
            if (node == null)
                return;

            if (node.type == NodeType.FRAME && IsScreenNode(node, parentNode))
                screenNodeIds.Add(node.id);
            ...
            {
                CollectScreenNodeIds(child, node, screenNodeIds);
            }
        }

        static bool NodeIsInSubtree(Node root, Node target)
        {
            ...
            {
                if (NodeIsInSubtree(child, target))
                    return true;
            }
