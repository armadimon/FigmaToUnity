...
namespace UnityToFigma.Editor.Nodes
{
    public static class FigmaNodeManager
    {
            ...
            if (node.fills.Length > 0)
            {
                ...
                switch (firstFill.type)
                {
                    case Paint.PaintType.IMAGE:
                        SetupImageFill(figmaImage, firstFill, figmaImportProcessData);
                        break;
        ...
        private static void SetupImageFill(FigmaImage figmaImage,Paint fill, FigmaImportProcessData figmaImportProcessData)
        {
            ...
            figmaImage.sprite = AssetDatabase.LoadAssetAtPath<Sprite>(
                    paths.GetPathForImageFill(fill.imageRef));
